...
// FFmpeg 路径 - 从 CSDM 配置文件读取
let FFMPEG_PATH = 'ffmpeg'; // 默认使用系统PATH中的ffmpeg
try {
  ...
  if (fs.existsSync(csdmSettingsPath)) {
    ...
    if (csdmSettings.video?.ffmpegSettings?.customLocationEnabled &&
        csdmSettings.video?.ffmpegSettings?.customExecutableLocation) {
      FFMPEG_PATH = csdmSettings.video.ffmpegSettings.customExecutableLocation;
      console.log(`[INFO] 使用 CSDM 配置的 FFmpeg 路径: ${FFMPEG_PATH}`);
    }
  }
} catch (err) {
  console.warn(`[警告] 无法读取 CSDM 配置文件，将使用系统 PATH 中的 ffmpeg: ${err.message}`);
}
...
async function main() {
  let client;
  let outputPath;  // 在try外部声明，以便catch块可以访问
  ...
  try {
    ...
    outputPath = path.join(demoDir, outputFolder);  // 改为赋值而不是声明
    ...
    if (await fs.pathExists(outputPath)) {
        try {
            ...
            // 同时检查 review_status 字段（新格式）和旧的 states/error 字段（向后兼容）
            if (videoExists && videoSize > 0 && frameCount > 0 && dataJson) {
                // 新格式：检查 review_status
                if (dataJson.review_status === 'rejected') {
                    isComplete = true;
                    console.log(`  ✓ 数据已标记为错误 (review_status: rejected)，跳过录制`);
                }
                else if (dataJson.review_status === 'approved') {
                    isComplete = true;
                    console.log(`  ✓ 数据已标记为合格 (review_status: approved)，跳过录制`);
                }
                // 旧格式兼容：检查 error: true 和 states: true
                else if (dataJson.error === true) {
                    isComplete = true;
                    console.log(`  ✓ 数据已标记为错误 (error: true)，跳过录制`);
                }
                else if (dataJson.states === true) {
                    isComplete = true;
                    console.log(`  ✓ 数据已标记为完成 (states: true)，跳过录制`);
                }
                else {
                    isComplete = true;
    ...
    console.log('');
    console.log('[DEBUG] ========== 倍速参数检查 ==========');
    console.log(`[DEBUG] 传入的speed参数: ${speed} (类型: ${typeof speed})`);
    console.log(`[DEBUG] parseInt后: ${parseInt(speed)}`);
    console.log(`[DEBUG] 将使用 CSDM 的 --speed 参数（录制倍速）`);
    console.log('[DEBUG] ======================================');
    console.log('');
    ...
    const hlaeCommand = [
      ...
      `--speed ${speed}`,  // 使用CSDM的--speed参数控制demo播放速度
      `--cfg "sv_cheats 1; cl_draw_only_deathnotices 0; host_timescale ${speed}"`,  // host_timescale控制游戏引擎时间流速
    ].join(' ');

    console.log('');
    console.log('[DEBUG] ========== 完整 HLAE 命令 ==========');
    console.log(hlaeCommand);
    console.log('[DEBUG] ========== 速度参数检查 ==========');
    console.log(`[DEBUG] --speed ${speed} (demo播放速度倍数)`);
    console.log(`[DEBUG] CFG commands: sv_cheats 1; cl_draw_only_deathnotices 0; host_timescale ${speed}`);
    console.log(`[DEBUG] host_timescale ${speed} (游戏引擎时间流速，需要sv_cheats 1)`);
    console.log('[DEBUG] =====================================');
    console.log('');
