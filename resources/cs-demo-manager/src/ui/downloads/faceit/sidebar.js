...
export function Sidebar() {
    ...
    return (React.createElement("div", { className: "min-w-fit overflow-auto border-r border-r-gray-300" }, matches.map((match) => {
        return (React.createElement(MatchEntry, { key: match.id, date: match.date, game: match.game, duration: match.durationInSeconds, isSelected: match.id === selectedMatchId, mapName: match.mapName, scoreOnTheLeft: match.teams[0].score, 
            // Sometimes there is only one team https://www.faceit.com/en/cs2/room/1-cf691eae-3371-4b7e-bfe2-785ba16d66a4/scoreboard
            scoreOnTheRight: match.teams[1]?.score ?? 0, selectMatch: () => {
