...
function RefreshMatchesButton() {
    ...
    const onClick = async () => {
        ...
        if (csIsRunning) {
            showDialog(React.createElement(CounterStrikeRunningDialog, { onConfirmClick: fetchLastMatches }));
        }
        else {
            fetchLastMatches();
        }
    };
    return React.createElement(RefreshButton, { onClick: onClick, isDisabled: isDisabled });
}
