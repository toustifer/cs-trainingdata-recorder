...
export function ChatMessages() {
    ...
    if (chatMessages.length === 0) {
        return React.createElement(EmptyChatMessages, null);
    }
    ...
    if (teamName !== '') {
        ...
        visibleChatMessages = visibleChatMessages.filter(({ senderSteamId }) => teamPlayers.some((player) => player.steamId === senderSteamId));
    }
    const options = [
        { value: '', label: React.createElement(Trans, null, "All teams") },
        { value: match.teamA.name, label: match.teamA.name },
        { value: match.teamB.name, label: match.teamB.name },
    ];
    return (React.createElement(React.Fragment, null,
        React.createElement(ActionBar, { left: React.createElement(ExportChatMessagesButton, { checksum: checksum, messages: visibleChatMessages }), right: React.createElement(React.Fragment, null,
                React.createElement(Select, { options: options, value: teamName, onChange: (teamName) => {
                        setTeamName(teamName);
                    } }),
                React.createElement(TextInputFilter, { value: fuzzySearchText, onChange: setFuzzySearchText })) }),
        React.createElement(ChatMessagesList, { chatMessages: visibleChatMessages })));
}
