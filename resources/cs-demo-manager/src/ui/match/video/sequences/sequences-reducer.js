...
import { addSequence, deleteSequence, deleteSequences, generatePlayersDeathsSequences, generatePlayersKillsSequences, generatePlayersRoundsSequences, replaceSequences, updateSequence, } from './sequences-actions';
...
export const sequencesReducer = createReducer(initialState, (builder) => {
    builder
        ...
        state[action.payload.demoFilePath] = sequences.filter((sequence) => sequence.number !== action.payload.sequence.number);
    })
        .addCase(updateSequence, (state, action) => {
        const sequences = state[action.payload.demoFilePath] ?? [];
        const sequenceToUpdateIndex = sequences.findIndex((sequence) => sequence.number === action.payload.sequence.number);
        ...
        const { match: { demoFilePath }, preserveExistingSequences, } = action.payload;
        ...
        const { match, steamIds, settings, startSecondsBeforeEvent, endSecondsAfterEvent, preserveExistingSequences } = action.payload;
...
//# sourceMappingURL=sequences-reducer.js.map