...
export function ResetSettingsButton() {
    ...
    const onClick = () => {
        const { recordingSystem, recordingOutput, closeGameAfterRecording, concatenateSequences, deathNoticesDuration, encoderSoftware, ffmpegSettings, framerate, height, width, } = defaultSettings.video;
        updateSettings({
            recordingSystem,
            recordingOutput,
    ...
    return (React.createElement(Button, { onClick: onClick },
        React.createElement(Trans, { context: "Button" }, "Reset settings")));
}
