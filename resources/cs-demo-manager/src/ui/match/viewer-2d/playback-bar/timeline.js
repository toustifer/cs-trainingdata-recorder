...
export function Timeline() {
    ...
    return (React.createElement("div", { className: "relative flex w-full border-x border-x-gray-300 bg-gray-50", ref: wrapperRef, onClick: onClick },
        React.createElement("div", { className: "bg-gray-300", style: {
                width: `${elapsedTimeWidth}px`,
            } }),
        React.createElement(FreezetimeEndIndicator, { leftX: tickToPlaybackBarX(round.freezetimeEndTick) }),
        kills.map((kill) => {
            const leftX = tickToPlaybackBarX(kill.tick);
            return React.createElement(KillIndicator, { key: kill.id, leftX: leftX, kill: kill });
        }),
        bombExploded !== null && (React.createElement(BombExplodedIndicator, { key: `playback-bomb-exploded-${bombExploded.id}`, leftX: tickToPlaybackBarX(bombExploded.tick), planterName: bombExploded.planterName, site: bombExploded.site, tick: bombExploded.tick })),
        bombPlanted !== null && (React.createElement(BombPlantedIndicator, { key: `playback-bomb-planted-${bombPlanted.id}`, leftX: tickToPlaybackBarX(bombPlanted.tick), planterName: bombPlanted.planterName, site: bombPlanted.site, tick: bombPlanted.tick }))));
}
