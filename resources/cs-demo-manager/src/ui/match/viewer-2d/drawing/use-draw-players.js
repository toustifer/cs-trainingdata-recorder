...
export function useDrawPlayers() {
    const { playerPositions, currentTick, focusedPlayerId, bombsDefuseStart, bombsPlantStart, tickrate, hostagesPickUpStart, hostagesPickedUp, } = useViewerContext();
    const bombImage = getBombImage();
    const drawPlayers = (context, { zoomedToRadarX, zoomedToRadarY, zoomedSize }) => {
        ...
        for (const position of playersAlivePositions) {
            ...
            const isHoldingGrenade = Object.values(GrenadeName).includes(position.activeWeaponName);
            if (isHoldingKnife || isHoldingGrenade) {
                ...
                if (isHoldingGrenade) {
                    ...
                    context.fillStyle = getGrenadeColor(position.activeWeaponName);
                    context.fill();
                }
            }
            else {
...
//# sourceMappingURL=use-draw-players.js.map