...
export function useDrawGrenades() {
    const { currentTick, tickrate, grenadePositions, smokesStart, decoysStart, heGrenadesExplode, flashbangsExplode } = useViewerContext();
    const drawGrenades = (context, { zoomedSize, zoomedToRadarX, zoomedToRadarY }) => {
        const grenadeCircleSize = zoomedSize(4);
        const drawSmokeEffect = (smokeStarted, smokeExpiredPosition) => {
        ...
        const drawDecoyEffect = (position) => {
        ...
        const drawHeGrenadeExplode = (heGrenadeExplode) => {
        ...
        for (const position of positions) {
            ...
            for (let i = 0; i < previousPositions.length - 1; i++) {
                ...
                context.moveTo(zoomedToRadarX(currentPosition.x, currentPosition.z), zoomedToRadarY(currentPosition.y, currentPosition.z));
...
//# sourceMappingURL=use-draw-grenades.js.map