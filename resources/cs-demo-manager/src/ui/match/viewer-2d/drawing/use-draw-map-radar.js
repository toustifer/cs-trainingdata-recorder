...
export function useDrawMapRadar() {
    ...
    const radarImage = useRef(null);
    const lowerRadarImage = useRef(null);
    ...
    const drawMapRadar = (context, { zoomedX, zoomedY, getScaledRadarSize, zoomedSize }) => {
        if (context === null) {
            return;
        }
        ...
        if (lowerRadarImage.current !== null) {
            ...
            context.drawImage(lowerRadarImage.current, x + scaledOffsetX, y + radarSize + scaledOffsetY, radarSize, radarSize);
            context.restore();
        }
...
//# sourceMappingURL=use-draw-map-radar.js.map