...
export function useDrawBombs() {
    ...
    const lastBombPosition = useRef(null);
    const drawBombs = (context, { zoomedToRadarX, zoomedToRadarY, zoomedSize }) => {
        if (bombPlanted === null || bombPlanted.tick > currentTick) {
            ...
            const drawBomb = ({ x, y, z }) => {
                const zoomedX = zoomedToRadarX(x, z);
                const zoomedY = zoomedToRadarY(y, z);
                const bombImageSize = zoomedSize(14);
                context.drawImage(bombImage, zoomedX - bombImageSize / 2, zoomedY - bombImageSize / 2, bombImageSize, bombImageSize);
            };
...
//# sourceMappingURL=use-draw-bombs.js.map