...
function buildColumnsWidthMapping(columns) {
    return Object.fromEntries(columns.map((column) => {
        return [column.id, column.width];
    }));
}
export function useTable({ data, columns, fuzzySearchText, fuzzySearchColumnIds, getRowId, rowSelection, onSelectionChanged, onKeyDown, onRowDoubleClick, onSelectWithKeyboard, onContextMenu, persistStateKey, persistScrollKey, getRowStyle, selectedRowIds, sortedColumn, }) {
    ...
    const columnOrderRef = useRef(columns.map((column) => column.id));
    const columnVisibilityRef = useRef({});
    const columnsWidthRef = useRef(buildColumnsWidthMapping(columns));
    ...
    useEffect(() => {
        ...
        const loadPersistedState = async () => {
            const columnsState = await window.csdm.readTableState(persistStateKey);
    ...
    const updateSelection = (rowIds) => {
        selectedRowIdsRef.current = rowIds;
        onSelectionChanged?.(table);
        render();
    };
    const getOrderedColumns = (visibleColumns) => {
        let orderedColumns = [];
    ...
    const persistTableState = () => {
        ...
        const columnsState = orderedColumns.map((column) => {
            return {
                id: column.id,
    ...
    const table = {
        ...
        getColumnProps: (column, index) => {
            ...
            const onResizerMouseMove = (event) => {
                if (!columnRef) {
                    return;
                }
        ...
        getRowProps: (rowId, row, rowIndex) => {
            return {
                onMouseDown: (event) => {
