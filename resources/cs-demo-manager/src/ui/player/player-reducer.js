...
import { fetchPlayerError, fetchPlayerStart, fetchPlayerSuccess, playerCommentUpdated, selectedMatchesChanged, } from './player-actions';
...
export const playerReducer = createReducer(initialState, (builder) => {
    builder
        ...
        if (state.player) {
            state.player.matches = state.player.matches.filter((match) => !action.payload.deletedChecksums.includes(match.checksum));
        }
        state.selectedMatchChecksums = state.selectedMatchChecksums.filter((checksum) => !action.payload.deletedChecksums.includes(checksum));
    })
