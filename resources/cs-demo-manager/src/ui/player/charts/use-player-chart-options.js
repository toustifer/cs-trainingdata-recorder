...
function useGetTimeAxisLabel() {
    ...
    const getTimeAxisLabel = (axis, timestamp, axisIndex) => {
        if (axis === 'match') {
...
export function usePlayerChartOptions({ title, data, axis, minValue, maxValue, name, yLineParallelToXAxisValue, }) {
            ...
            axisPointer: {
                label: {
                    formatter: ({ value, seriesData }) => {
                        return getTimeAxisLabel(axis, value, seriesData[0].dataIndex);
                    },
                },
            },
            axisLine: {
...
//# sourceMappingURL=use-player-chart-options.js.map