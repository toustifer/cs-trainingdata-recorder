...
function computeValuesAverage(percentageA, percentageB) {
 ...
 * */
export function buildAveragePlayerChartData({ field, chartsData, axis }) {
    ...
    for (let i = 0; i < chartsData.length; i++) {
        ...
        const previousChartData = chartsData[i - 1];
        const previousMatchDate = new Date(previousChartData.matchDate);
        let previousMatchTimestamp;
        switch (axis) {
            case 'day':
...
//# sourceMappingURL=build-average-player-chart-data.js.map