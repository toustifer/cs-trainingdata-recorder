...
export function CurrentFolderSelect() {
    ...
    return (React.createElement("div", { className: "flex items-center gap-12 p-8" },
        React.createElement(FolderIcon, { height: 24 }),
        React.createElement("div", { className: "flex w-full flex-col" },
            React.createElement(Select, { isDisabled: !demosLoaded || showAllFolders || folders.length === 0, onChange: async (selectedFolderPath) => {
                    ...
                    fetchDemos();
                }, value: currentFolderPath, options: folders.map((folder) => {
                    return {
                        value: folder.path,
                        label: folder.path,
                    };
                }) })),
        folder && (React.createElement(Checkbox, { label: React.createElement(Trans, { context: "Checkbox label" }, "Include subfolders"), isChecked: folder.includeSubFolders, isDisabled: showAllFolders || !canInteract, onChange: async (event) => {
                await toggleFolderSubFoldersInclusion(folder.path, event.target.checked);
                await fetchDemos();
            } })),
        React.createElement(Checkbox, { label: React.createElement(Trans, { context: "Checkbox label" }, "See all folders"), isChecked: showAllFolders, isDisabled: !canInteract, onChange: async (event) => {
...
//# sourceMappingURL=current-folder-select.js.map