...
export function useProcessDemosToAnalyze() {
    ...
    return (demos) => {
        ...
        const isSelectionAnalyzable = demosAlreadyInDatabase.length === 0 && demosAnalysesInProgressCount === 0 && demosSourceNotSupportedCount === 0;
        if (isSelectionAnalyzable) {
            addDemosToAnalyses(analyzableDemos);
        }
        else {
            showDialog(React.createElement(AnalyzeConfirmationDialog, { analyzableDemos: analyzableDemos, demosAlreadyInDatabase: demosAlreadyInDatabase, demosAnalysesInProgressCount: demosAnalysesInProgressCount, demosSourceNotSupportedCount: demosSourceNotSupportedCount }));
        }
    };
}
//# sourceMappingURL=use-process-demos-to-analyze.js.map