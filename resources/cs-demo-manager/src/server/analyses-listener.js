...
class AnalysesListener {
    analyses = [];
    currentAnalyses = [];
    outputFolderPath; // Folder path where CSV files will be write on the host
    constructor() {
        this.outputFolderPath = path.resolve(os.tmpdir(), 'cs-demo-manager');
    }
    removeDemosByChecksums(checksums) {
    ...
    async addDemosToAnalyses(demos) {
    ...
    async loopUntilAnalysesDone() {
        const promises = [];
        const settings = await getSettings();
        const maxConcurrentAnalyses = Math.min(MAX_CONCURRENT_ANALYSES, settings.analyze.maxConcurrentAnalyses ?? MAX_CONCURRENT_ANALYSES / 2);
        while (this.analyses.length > 0 && this.currentAnalyses.length < maxConcurrentAnalyses) {
            ...
            if (analysis) {
                ...
                const analysisPromise = this.processAnalysis(analysis, settings.analyze.analyzePositions)
                    ...
                    .finally(() => {
                    this.currentAnalyses = this.currentAnalyses.filter(({ demoChecksum }) => demoChecksum !== analysis.demoChecksum);
                });
    ...
    processAnalysis = async (analysis, analyzePositions) => {
    ...
    async insertMatch(analysis, checksum, demoPath) {
        try {
    ...
    updateAnalysisStatus = (analysis, status, errorCode) => {
