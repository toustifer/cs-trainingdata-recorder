...
class DownloadDemoQueue {
    ...
    writeMatchInfoFile = async (protobufBytes, infoFilePath) => {
        try {
    ...
    isMatchAlreadyInQueue(matchId) {
        return (this.currentDownload?.matchId === matchId ||
            this.downloads.some((download) => {
                return download.matchId === matchId;
            }));
    }
    isMatchAlreadyDownloaded = async (downloadFolderPath, download) => {
    ...
    buildDemoPath(downloadFolderPath, fileName) {
        const demoPath = path.join(downloadFolderPath, `${fileName}.dem`);
        return demoPath;
    }
    buildDemoInfoFilePath(demoPath) {
        return `${demoPath}.info`;
    }
    loadDownloadedDemo = async (demoPath) => {
        try {
