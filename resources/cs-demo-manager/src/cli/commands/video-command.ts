...
export class VideoCommand extends Command {
  ...
  private readonly speedFlag = 'speed';
  ...
  public printHelp() {
    ...
    console.log(`  --${this.speedFlag} <number> (playback speed multiplier, default: 40)`);
  ...
  private buildCfg(): string | undefined {
    let cfg = this.cfg || '';

    if (this.speed !== undefined) {
      const timescaleCmd = `demo_timescale ${this.speed}; host_timescale ${this.speed}`;
      cfg = cfg ? `${cfg}; ${timescaleCmd}` : timescaleCmd;
      console.log(`[DEBUG] Speed parameter: ${this.speed}`);
      console.log(`[DEBUG] Generated cfg: ${cfg}`);
    }
  ...
  protected async parseArgs() {
    ...
    if (values[this.speedFlag]) {
      const speed = Number(values[this.speedFlag]);
      if (Number.isNaN(speed)) {
        throw new InvalidArgument('Speed is not a number');
      }
      if (speed <= 0) {
        throw new InvalidArgument('Speed must be greater than 0');
      }
      this.speed = speed;
    }
