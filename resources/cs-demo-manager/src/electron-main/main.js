...
app.commandLine.appendSwitch('disable-features', ['UseEcoQoSForBackgroundProcess', 'IntensiveWakeUpThrottling'].join(','));
async function start() {
    await app.whenReady();
    const sendNavigateToDemo = (mainWindow, demoPath) => {
        mainWindow.webContents.send(IPCChannel.OpenDemFile, demoPath);
        mainWindow.show();
    };
    const getDemoPathFromArguments = (args) => {
    ...
    ipcMain.handle(IPCChannel.LocaleChanged, async (event, locale) => {
    ...
    const onBeforeQuit = async (event) => {
        ...
        const hasPendingAnalyses = await client.send({
