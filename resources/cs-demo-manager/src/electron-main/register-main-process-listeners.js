...
function sanitizePathForFileSelection(path) {
    return isWindows ? path.replace(/\//g, '\\') : path;
}
export function registerMainProcessListeners() {
    ipcMain.handle(IPCChannel.BrowseToFile, (event, path) => {
        shell.showItemInFolder(sanitizePathForFileSelection(path));
    });
    ipcMain.handle(IPCChannel.BrowseToFolder, async (event, folderPath) => {
        try {
            /**
    ...
    ipcMain.handle(IPCChannel.ShowSaveDialog, (event, options) => {
        const mainWindow = windowManager.getOrCreateMainWindow();
        return dialog.showSaveDialog(mainWindow, options);
    });
    ipcMain.handle(IPCChannel.ShowOpenDialog, (event, options) => {
        const mainWindow = windowManager.getOrCreateMainWindow();
    ...
    ipcMain.handle(IPCChannel.UpdateSystemStartupBehavior, async (event, behavior) => {
        await updateSystemStartupBehavior(behavior);
    });
}
//# sourceMappingURL=register-main-process-listeners.js.map