...
async function convertGameAudioFile(ffmpegExecutablePath, wavFilePath, audioCodec, audioBitrate, signal) {
    const codecs = [
        { name: 'mp3', ext: '.mp3' },
        { name: 'aac', ext: '.m4a' },
        { name: 'alac', ext: '.m4a' },
...
export async function generateVideoWithFFmpeg(settings, signal) {
    logger.debug('Generating video with FFmpeg');
    logger.debug(settings);
    const { tgaFiles, wavFilePath, videoFilePath } = await getSequenceRawFiles(settings);
    const { ffmpegExecutablePath, framerate, inputParameters, videoCodec, audioCodec, audioBitrate, constantRateFactor, outputParameters, recordingSystem, game, sequence, outputFolderPath, videoContainer, recordingOutput, recordAudio, } = settings;
    ...
    if (recordingSystem === RecordingSystem.HLAE && recordingOutput === RecordingOutput.Video) {
        ...
        if (hasAudio) {
            ...
            const audioFilePath = await convertGameAudioFile(ffmpegExecutablePath, wavFilePath, audioCodec, audioBitrate, signal);
    ...
    if (recordingSystem === RecordingSystem.HLAE) {
        ...
        rawFilesPathPattern = path.join(sequenceRawFilesFolderPath, game === Game.CSGO ? `${sequenceName}%04d.tga` : `${sequenceName}%08d.tga`);
    }
    const args = [
        '-y', // override the file if it exists
        `-framerate ${framerate}`,
...
//# sourceMappingURL=generate-video-with-ffmpeg.js.map