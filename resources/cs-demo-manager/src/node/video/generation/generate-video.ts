...
import { ensureCs2PluginInstalled } from 'csdm/electron-main/ensure-cs2-plugin';
import { withRetryOnTimeout } from './with-retry-on-timeout';
...
export async function generateVideo(parameters: Parameters) {
  ...
  if (game === Game.CSGO) {
    ...
    const players = await fetchMatchPlayersSlots(checksum).catch((error) => {
      logger.warn(`Failed to fetch match players: ${error.message}, using fallback mode`);
  ...
  // 录制前确保 CS2 插件已安装并配置正确
  if (game === Game.CS2) {
    logger.log('Checking CS2 plugin before recording...');
    await ensureCs2PluginInstalled();
  }

  const shouldGenerateVideo = recordingOutput !== RecordingOutput.Images;
  try {
    // 添加超时重试机制：如果录制卡死，自动重试最多3次
    await withRetryOnTimeout(
      ...
      {
        maxRetries: 3,
        timeoutMs: 90 * 1000, // 180秒超时（3分钟）
        taskName: `Recording sequences (videoId: ${videoId})`,
        onRetry: (attempt) => {
          logger.warn(`Recording attempt ${attempt} failed or timed out, retrying...`);
        },
      },
    );
