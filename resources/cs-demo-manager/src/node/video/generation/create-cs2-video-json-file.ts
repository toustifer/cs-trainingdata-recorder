...
export async function createCs2VideoJsonFile({
  ...
  for (let i = 0; i < sequences.length; i++) {
    ...
    // 在游戏启动时（tick 1）就设置倍速，避免跳转过程中使用默认高速
    if (typeof sequence.cfg === 'string') {
      const commands = sequence.cfg.split('\n');
      for (const command of commands) {
        // 提取倍速相关命令，在 tick 1 就执行
        if (command.includes('timescale') || command.includes('host_framerate')) {
          json.addExecCommand(1, command);
        }
      }
    }
    ...
    // 延迟skipAhead到tick 68，让tick 64的timescale命令有时间生效
    json.addSkipAhead(68, Math.max(1, setupSequenceTick - 1));
    ...
    for (const camera of sequence.playerCameras) {
      ...
      if (player) {
        ...
        // Fallback: 如果没有找到 player slot，直接使用 Steam ID
        // CS2 的 spec_player 命令也可以接受 Steam ID（需要加 'x' 前缀）
        json.addSpecPlayer(camera.tick, `x${camera.playerSteamId}`);
      }
    }
