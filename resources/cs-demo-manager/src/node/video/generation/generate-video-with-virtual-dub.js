...
function generateVirtualDubScript(framerate, firstTgaFilePath, wavFilePath, tgaFilesCount, videoFilePath) {
    return `
...
async function writeVirtualDubScript({ recordingSystem, sequence, outputFolderPath, game, framerate, recordAudio, }) {
    ...
    const script = generateVirtualDubScript(framerate, sanitizePath(tgaFiles[0]), recordAudio && wavFilePath && wavFileExists ? sanitizePath(wavFilePath) : undefined, tgaFiles.length, outputFilePath);
    const scriptFilePath = path.join(getVirtualDubFolderPath(), SCRIPT_FILENAME);
    await fs.writeFile(scriptFilePath, script);
}
async function executeVirtualDub(signal) {
...
export async function generateVideoWithVirtualDub(settings, signal) {
    await fs.ensureDir(settings.outputFolderPath);
    await writeVirtualDubScript(settings);
    await executeVirtualDub(signal);
}
//# sourceMappingURL=generate-video-with-virtual-dub.js.map