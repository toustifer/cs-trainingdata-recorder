...
function getHlaeOutputFolderPath(outputFolderPath, sequence) {
 ...
 */
function buildReplacePlayerNameCommand(player) {
    return `mirv_exec mirv_replace_name filter add x${player.steamId} "${player.playerName.replaceAll('"', '{QUOTE}')}"`;
}
export async function createCsgoVideoJsonFile({ type, recordingSystem, recordingOutput, encoderSoftware, outputFolderPath, framerate, demoPath, sequences, closeGameAfterRecording, tickrate, ffmpegSettings, }) {
    ...
    for (let i = 0; i < sequences.length; i++) {
        ...
        const presetName = recordingOutput === RecordingOutput.Video && encoderSoftware === EncoderSoftware.FFmpeg
        ...
        if (presetName !== 'afxClassic') {
            ...
            json
                .addExecCommand(setupSequenceTick, `mirv_streams settings add ffmpeg ${presetName} "${presetParameters} {QUOTE}${hlaeOutputFolderPath}\\\\video.${ffmpegSettings.videoContainer}{QUOTE}"`)
        ...
        for (const playerOptions of sequence.playersOptions) {
            ...
            if (!playerOptions.showKill) {
                json.addExecCommand(setupSequenceTick, `mirv_deathmsg filter add attackerMatch=x${playerOptions.steamId} block=1`);
            }
            if (playerOptions.highlightKill) {
                json.addExecCommand(setupSequenceTick, `mirv_deathmsg filter add attackerMatch=x${playerOptions.steamId} attackerIsLocal=1`);
            }
        }
        if (type === 'record') {
...
//# sourceMappingURL=create-csgo-video-json-file.js.map