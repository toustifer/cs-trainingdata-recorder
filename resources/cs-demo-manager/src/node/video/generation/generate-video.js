...
async function buildVideos({ signal, ...options }) {
    throwIfAborted(signal);
    const { recordingSystem, recordingOutput, encoderSoftware, framerate, sequences, outputFolderPath, concatenateSequences, ffmpegSettings, game, onSequenceStart, onConcatenateSequencesStart, } = options;
...
export async function generateVideo(parameters) {
    logger.debug('Generating video with parameters:');
    logger.debug(parameters);
    const { checksum, videoId, recordingSystem, recordingOutput, encoderSoftware, framerate, demoPath, width, height, closeGameAfterRecording, tickrate, game, ffmpegSettings, outputFolderPath, signal, onMoveFilesStart, } = parameters;
    throwIfAborted(signal);
