...
function buildUnixCommand(scriptPath, args, game) {
    if (game === Game.CSGO) {
        return `${scriptPath} --args ${args}`;
    }
    return `${scriptPath} ${args}`;
}
async function buildLinuxCommand(scriptPath, args, game, settings) {
    ...
    if (settings.cs2SteamRuntimeScriptPath) {
        ...
        if (result.length === 0) {
            logger.error(`Cannot find the Steam Linux Runtime script "${steamScriptName}" in the Steam folder at ${steamFolderPath}`);
            throw new CounterStrikeExecutableNotFound(game);
        }
...
function buildWindowsCommand(executablePath, args) {
    return `"${executablePath}" ${args}`;
}
async function buildCommand(executablePath, args, game, settings) {
    if (isWindows) {
...
export async function startCounterStrike(options) {
...
//# sourceMappingURL=start-counter-strike.js.map