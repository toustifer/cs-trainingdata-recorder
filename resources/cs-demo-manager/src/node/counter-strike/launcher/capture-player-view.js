...
function assertScreenshotFilesNotEmpty(paths) {
    if (paths.length === 0) {
        throw new RawFilesNotFoundError();
    }
}
async function assertRecordingFolderExists(folderPath) {
    const exists = await fs.pathExists(folderPath);
    if (!exists) {
        throw new RawFilesNotFoundError();
    }
}
async function deleteScreenshotFiles(game, recordingFolderPath) {
    if (game === Game.CSGO) {
...
export async function capturePlayerView(game) {
...
//# sourceMappingURL=capture-player-view.js.map