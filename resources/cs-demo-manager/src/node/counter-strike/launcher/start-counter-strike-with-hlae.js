...
async function registerFfmpegLocation(hlaeExecutablePath) {
...
function getGameProcessName(game) {
    return game === Game.CSGO ? 'csgo.exe' : 'cs2.exe';
}
async function isHlaeErrorWindowExists(game) {
    return new Promise((resolve) => {
        return exec(`tasklist /fi "imagename eq ${getGameProcessName(game)}" /v /nh`, { windowsHide: true }, (err, stdout) => {
            return resolve(stdout.includes('Error - AfxHookSource'));
        });
    });
}
async function startHlae({ command, signal, game }) {
...
export async function startCounterStrikeWithHlae(options) {
    if (!isWindows) {
    ...
    const { width: userWidth, height: userHeight, fullscreen: userFullscreen, launchParameters: userLaunchParameters, } = settings.playback;
    ...
    if (game === Game.CSGO) {
        hlaeParameters.push('-csgoLauncher', `-csgoExe "${csExecutablePath}"`, `-customLaunchOptions "${launchParameters.join(' ')}"`);
    }
    else {
        hlaeParameters.push('-customLoader', `-hookDllPath "${path.join(path.dirname(hlaeExecutablePath), 'x64', 'AfxHookSource2.dll')}"`, `-programPath "${csExecutablePath}"`, `-cmdLine "${launchParameters.join(' ')}"`);
    }
...
//# sourceMappingURL=start-counter-strike-with-hlae.js.map