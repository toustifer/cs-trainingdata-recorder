...
async function fetchRounds(checksum, steamId) {
    const rows = await db
        .selectFrom('rounds')
...
async function fetchMatchTickrate(checksum) {
    const row = await db
        .selectFrom('matches')
        .select(['tickrate'])
...
export async function watchPlayerRounds({ demoPath, steamId, onGameStart }) {
...
//# sourceMappingURL=watch-player-rounds.js.map