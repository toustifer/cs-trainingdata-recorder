...
async function getWindowsCsExecutablePath(csgoFolderPath, game) {
    const executablePath = game === Game.CSGO
...
async function getLinuxCsRunScriptPath(csgoFolderPath, game) {
    const scriptPath = game === Game.CSGO ? path.join(csgoFolderPath, 'csgo.sh') : path.join(csgoFolderPath, 'game', 'cs2.sh');
...
async function getMacosCsgoRunScriptPath(csgoFolderPath) {
...
export async function getCounterStrikeExecutablePath(game) {
...
//# sourceMappingURL=get-counter-strike-executable-path.js.map