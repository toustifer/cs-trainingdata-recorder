...
export async function updateValvePlayersFromSteam(players) {
    try {
        const steamIds = players.map((player) => player.steamId);
        let accounts = await fetchSteamAccounts(steamIds);
        const needsToFetchPlayersFromSteam = steamIds.filter((steamId) => {
...
//# sourceMappingURL=update-valve-players-from-steam.js.map