...
export async function getUsersBan(steamIds) {
    const bans = [];
    let promises = [];
    ...
    for (let i = 0; i < steamIds.length; i += MAX_STEAM_IDS_PER_REQUEST) {
        ...
        promises.push((async () => {
            const response = await fetch(`https://api.steampowered.com/ISteamUser/GetPlayerBans/v1/?key=${steamApiKey}&steamids=${ids}`);
...
//# sourceMappingURL=get-players-bans.js.map