...
export async function fetchTeamShotsPoints(filters) {
    let query = db
        .selectFrom('shots')
        .select(['x', 'y'])
    ...
    if (filters.players.length > 0) {
        query = query.where('player_steam_id', 'in', filters.players.map((player) => player.steamId));
    }
...
//# sourceMappingURL=fetch-team-shots-points.js.map