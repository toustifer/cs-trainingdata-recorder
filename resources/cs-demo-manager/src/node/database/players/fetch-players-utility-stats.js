...
async function fetchPlayersAverageBlindTime(steamIds, filters) {
    let query = db
        ...
        .select(() => {
        return [sql `ROUND(AVG(blinds.duration)::numeric, 1)`.as('average_duration')];
    })
...
async function fetchPlayersAverageEnemiesFlashed(steamIds, filters) {
    const query = db
        ...
        .select(() => [
        sql `ROUND((flashed_count::NUMERIC / NULLIF(total_count, 0)::NUMERIC), 1)`.as('average_enemies_flashed'),
    ])
...
async function fetchPlayersAverageHeGrenadeDamage(steamIds, filters) {
    const query = db
        ...
        .select(sql `ROUND((total_health_damage::NUMERIC / NULLIF(total_count, 0)::NUMERIC), 1)`.as('average_damage'))
...
async function fetchPlayersAverageSmokesThrownPerMatch(steamIds, filters) {
    const query = db
        ...
        .select(sql `AVG(total_count)::numeric(10,1)`.as('average_smokes_thrown_per_match'))
        .orderBy('steamId')
        .groupBy('steamId');
    const rows = await query.execute();
    return rows;
}
export async function fetchPlayersUtilityStats(steamIds, filters) {
    const [averageBlindTime, averageEnemiesFlashed, averageHeGrenadeDamage, averageSmokesThrownPerMatch] = await Promise.all([
...
//# sourceMappingURL=fetch-players-utility-stats.js.map