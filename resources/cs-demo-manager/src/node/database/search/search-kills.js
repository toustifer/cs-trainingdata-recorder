...
export async function searchKills({ headshot, noScope, wallbang, jump, throughSmoke, teamKill, collateralKill, steamIds, victimSteamIds, mapNames, startDate, endDate, demoSources, roundTagIds, matchTagIds, weaponNames, }) {
    let query = db
        .selectFrom('kills')
        .selectAll('kills')
        .distinct()
    ...
    if (collateralKill !== TriStateFilter.All) {
        query = query
            ...
            .having(sql `COUNT(*)`, collateralKill === TriStateFilter.Yes ? '>' : '=', 1);
    }
...
//# sourceMappingURL=search-kills.js.map