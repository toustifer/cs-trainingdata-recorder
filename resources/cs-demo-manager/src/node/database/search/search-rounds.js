...
export async function searchRounds({ steamIds, mapNames, startDate, endDate, demoSources, roundTagIds, matchTagIds, }) {
    let query = db
        .selectFrom('rounds')
        .selectAll('rounds')
        .distinct()
        ...
        .select(sql `ARRAY_AGG(DISTINCT round_tags.tag_id) FILTER (WHERE round_tags.tag_id IS NOT NULL)`.as('tagIds'))
...
//# sourceMappingURL=search-rounds.js.map