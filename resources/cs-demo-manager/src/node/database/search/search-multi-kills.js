...
export async function searchMultiKills({ killCount, steamIds, victimSteamIds, weaponNames, mapNames, startDate, endDate, demoSources, roundTagIds, matchTagIds, }) {
    let query = db
        .selectFrom('kills as k1')
        .selectAll(['k1'])
        .select([sql `COUNT(DISTINCT k1.id)`.as('killCount')])
        .distinct()
...
//# sourceMappingURL=search-multi-kills.js.map