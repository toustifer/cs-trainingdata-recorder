...
export async function fetchMatchPlayers(checksum) {
    const rows = await db
        .selectFrom('players')
        .select([
    ...
    sql `CASE WHEN steam_accounts.last_ban_date > matches.date AND ignored_steam_accounts.steam_id IS NULL THEN steam_accounts.last_ban_date END`.as('last_ban_date'))
        ...
        .select(sql `COUNT(kills.id) FILTER (WHERE kills.penetrated_objects > 0 AND kills.killer_steam_id = players.steam_id)`.as('wallbangKillCount'))
        .select(sql `COUNT(kills.id) FILTER (WHERE kills.is_no_scope = true AND kills.killer_steam_id = players.steam_id)`.as('noScopeKillCount'))
...
//# sourceMappingURL=fetch-match-players.js.map