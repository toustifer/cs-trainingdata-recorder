...
export async function fetchMatchesByChecksums(checksums) {
    const rows = await db
        .selectFrom('matches')
        .selectAll('matches')
    ...
    for (const row of rows) {
        ...
        const [players, rounds, kills, clutches, bombsPlanted, bombsDefused, bombsExploded, damages, blinds, playersEconomies, shots, chatMessages, chickenDeaths, teamsEconomyStats,] = await Promise.all([
...
//# sourceMappingURL=fetch-matches-by-checksums.js.map