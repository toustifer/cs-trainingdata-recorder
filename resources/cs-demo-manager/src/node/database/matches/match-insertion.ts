...
export async function insertFromCsv<Table>({
  ...
  // On Windows, psql requires backslash paths, not forward slashes
  const normalizedCsvPath = process.platform === 'win32'
    ? csvFilePath.replace(/\//g, '\\')
    : csvFilePath;

  const command = `-c "\\copy ${tableName}(${columnNames}) FROM '${normalizedCsvPath}' ENCODING 'UTF8' CSV DELIMITER ','" "postgresql://${username}:${encodeURIComponent(
    password,
