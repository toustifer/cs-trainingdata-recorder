...
export async function fetchTeamBombsStats(filters) {
    ...
    const query = db
        ...
        let query = db
            ...
            .innerJoin('players', (join) => join.on((eb) => eb.and([
        ...
        return [
            eb
                .selectFrom('team_rounds')
                .select(eb.fn.count('team_rounds.winner_name').as('roundsWonDueToBombExplosion'))
                .innerJoin('bomb_exploded_rounds', (join) => join.on((eb) => eb.and([
            ...
            eb
                .selectFrom('team_rounds')
                .select(eb.fn.count('team_rounds.winner_name').as('roundsWonDueToDefusal'))
                .innerJoin('bomb_defused_rounds', (join) => join.on((eb) => eb.and([
            ...
            eb
                .selectFrom('team_rounds')
                .select(eb.fn.count('team_rounds.winner_name').as('roundsLostDueToBombExplosion'))
                .innerJoin('bomb_exploded_rounds', (join) => join.on((eb) => eb.and([
                eb('team_rounds.match_checksum', '=', eb.ref('bomb_exploded_rounds.match_checksum')),
                eb('team_rounds.round_number', '=', eb.ref('bomb_exploded_rounds.round_number')),
            ])))
            ...
            eb
                .selectFrom('team_rounds')
                .select(eb.fn.count('team_rounds.winner_name').as('roundsLostDueToDefusal'))
                .innerJoin('bomb_defused_rounds', (join) => join.on((eb) => eb.and([
                eb('team_rounds.match_checksum', '=', eb.ref('bomb_defused_rounds.match_checksum')),
                eb('team_rounds.round_number', '=', eb.ref('bomb_defused_rounds.round_number')),
            ])))
            ...
            eb.selectFrom('bomb_plants').select(eb.fn.countAll().as('plantCount')).as('plantCount'),
            eb
                .selectFrom('bomb_plants')
                .select(eb.fn.countAll().as('plantCountSiteA'))
                .where('bomb_plants.site', '=', 'A')
                .as('plantCountSiteA'),
            eb
                .selectFrom('bomb_plants')
                .select(eb.fn.countAll().as('plantCountSiteB'))
                .where('bomb_plants.site', '=', 'B')
                .as('plantCountSiteB'),
            eb
                .selectFrom('team_rounds')
                .select(eb.fn.count('team_rounds.winner_name').as('roundsWonByPlayerDeaths'))
                .where('team_rounds.winner_name', '=', teamName)
                .where(({ eb, refTuple, selectFrom }) => eb(refTuple('team_rounds.match_checksum', 'team_rounds.round_number'), 'in', selectFrom('bomb_plants')
                .select(['match_checksum', 'round_number'])
                .$asTuple('match_checksum', 'round_number')))
                .where(({ eb, refTuple, selectFrom }) => eb(refTuple('team_rounds.match_checksum', 'team_rounds.round_number'), 'not in', selectFrom('bomb_exploded_rounds')
                .select(['match_checksum', 'round_number'])
                .$asTuple('match_checksum', 'round_number')))
                .where(({ eb, refTuple, selectFrom }) => eb(refTuple('team_rounds.match_checksum', 'team_rounds.round_number'), 'not in', selectFrom('bomb_defused_rounds')
                .select(['match_checksum', 'round_number'])
                .$asTuple('match_checksum', 'round_number')))
                .as('roundsWonByPlayerDeaths'),
            eb
                .selectFrom('team_rounds')
                .select(eb.fn.count('team_rounds.winner_name').as('roundsLostDueToPlayerDeaths'))
                .where('team_rounds.winner_name', '!=', teamName)
                .where(({ eb, refTuple, selectFrom }) => eb(refTuple('team_rounds.match_checksum', 'team_rounds.round_number'), 'in', selectFrom('bomb_plants_enemy')
                .select(['match_checksum', 'round_number'])
                .$asTuple('match_checksum', 'round_number')))
                .where(({ eb, refTuple, selectFrom }) => eb(refTuple('team_rounds.match_checksum', 'team_rounds.round_number'), 'not in', selectFrom('bomb_exploded_rounds')
                .select(['match_checksum', 'round_number'])
...
//# sourceMappingURL=fetch-team-bombs-stats.js.map