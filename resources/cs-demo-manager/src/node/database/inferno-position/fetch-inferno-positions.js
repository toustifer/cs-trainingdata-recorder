...
export async function fetchInfernoPositions(checksum, roundNumber) {
    const rows = await db
        ...
        .orderBy('tick')
        .execute();
    const infernoPositions = fillMissingTicks(rows.map(infernoPositionRowToInfernoPosition));
    return infernoPositions;
}
//# sourceMappingURL=fetch-inferno-positions.js.map