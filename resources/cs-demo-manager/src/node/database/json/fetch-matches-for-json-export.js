...
export async function fetchMatchesForJsonExport(checksums) {
    const rows = await db
        .selectFrom('matches')
        .selectAll('matches')
    ...
    for (const row of rows) {
        ...
        const [players, rounds, kills, clutches, bombsPlanted, bombsDefused, bombsExploded, damages, blinds, playersEconomies, shots, chatMessages, chickenDeaths, grenadeDestroyed,] = await Promise.all([
...
//# sourceMappingURL=fetch-matches-for-json-export.js.map