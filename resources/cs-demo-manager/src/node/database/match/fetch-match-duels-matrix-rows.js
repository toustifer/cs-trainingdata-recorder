...
export async function fetchMatchDuelsMatrixRows(checksum) {
    const result = await db
        ...
        sql `COUNT(DISTINCT k.id)`.as('killCount'),
        sql `COUNT(DISTINCT k2.id)`.as('deathCount'),
    ])
        .groupBy([
        'p1.steam_id',
        'p1.team_name',
...
//# sourceMappingURL=fetch-match-duels-matrix-rows.js.map