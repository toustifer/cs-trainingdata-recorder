...
export async function fetchPlayerOpeningDuelsStats(steamId, filters) {
    const query = db
        ...
        return db
            ...
            'kills.id',
            'kills.weapon_name',
            sql `CASE WHEN killer_steam_id = ${sql `${steamId}`} THEN 1 ELSE 0 END`.as('is_success'),
            sql `CASE WHEN is_trade_death = true THEN 1 ELSE 0 END`.as('is_traded'),
            sql `CASE WHEN killer_steam_id = ${sql `${steamId}`} THEN killer_side ELSE victim_side END`.as('player_side'),
        ])
        ...
        sql `ROUND(SUM(is_success)::numeric / COUNT(*) * 100)`.as('success_percentage'),
        sql `ROUND(SUM(is_traded)::numeric / COUNT(*) * 100)`.as('trade_percentage'),
    ])
    ...
    const buildTeamResult = (team) => {
        ...
        return {
            ...
            bestWeapon: (row?.best_weapon ?? WeaponName.Unknown),
        };
    };
    return {
...
//# sourceMappingURL=fetch-player-opening-duels-stats.js.map