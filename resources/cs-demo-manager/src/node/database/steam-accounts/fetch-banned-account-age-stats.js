...
export async function fetchBannedAccountAgeStats(ignoreBanBeforeFirstSeen) {
    let query = db
        .selectFrom('steam_accounts')
        .select([
        sql `CURRENT_DATE - AVG(AGE(NOW(), creation_date))`.as('average'),
        sql `CURRENT_DATE - percentile_cont(0.5) WITHIN GROUP (ORDER BY AGE(NOW(), creation_date))`.as('median'),
    ])
...
//# sourceMappingURL=fetch-banned-account-age-stats.js.map