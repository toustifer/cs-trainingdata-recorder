...
// 获取单个玩家的位置数据（用于训练数据导出，避免内存溢出）
export async function fetchSinglePlayerPositions(
  checksum: string,
  roundNumber: number,
  ...
  // 不使用 fillMissingTicks 以节省内存
  return rows.map(playerPositionRowToPlayerPosition);
}
