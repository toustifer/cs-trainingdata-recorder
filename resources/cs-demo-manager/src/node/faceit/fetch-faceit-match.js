...
function isFactionV1(faction) {
    return faction.roster_v1 !== undefined;
}
function getAvatarUrlPerPlayerId(matchDTO) {
    const avatarPerPlayerId = {};
...
async function buildFaceitMatchFromFaceitDTOs(matchDTO, teamsStatsDTO, downloadFolderPath) {
    ...
    for (const team of firstRoundStats.teams) {
        ...
        const avatarUrlPerPlayerId = getAvatarUrlPerPlayerId(matchDTO);
        teams.push({
            id: team.team_id,
    ...
    const downloadStatus = await getDownloadStatus(downloadFolderPath, matchDTO.match_id, demoUrl);
...
export async function fetchFaceitMatch(matchId, apiKey, downloadFolderPath) {
    const [matchDTO, statsDTO] = await Promise.all([
...
//# sourceMappingURL=fetch-faceit-match.js.map