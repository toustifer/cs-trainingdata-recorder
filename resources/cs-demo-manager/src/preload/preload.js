...
const api = {
    ...
    updateSystemStartupBehavior: async (behavior) => {
    ...
    onOpenDemoFile: (callback) => {
        ipcRenderer.on(IPCChannel.OpenDemFile, (event, demoPath) => {
    ...
    onToggleSettingsVisibility: (callback) => {
    ...
    showSaveDialog: async (options) => {
        const result = await ipcRenderer.invoke(IPCChannel.ShowSaveDialog, options);
        return result;
    },
    showOpenDialog: async (options) => {
        const result = await ipcRenderer.invoke(IPCChannel.ShowOpenDialog, options);
        return result;
    },
    browseToFolder: (folderPath) => {
    ...
    onNavigateToPendingDownloads: (callback) => {
    ...
    toggleAutoDownloadUpdates: (isEnabled) => {
    ...
    getDemoAudioData,
    getCounterStrikeLogFilePath: async (game) => {
        try {
