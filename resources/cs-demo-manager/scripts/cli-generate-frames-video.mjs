...
/**
 * CLI 视频版本的帧生成命令 - 方案三：Video + Pre-decoded Cache
 *
 * 优势：
 * - 存储空间节省 80-90% (视频压缩)
 * - 支持 LRU 缓存加速训练
 ...
 *   │   ├── video.avi          (完整视频)
 *   │   ├── frames_index.json  (帧索引：帧号->时间戳映射)
 *   │   └── data.json          (窗口信息 + 帧号引用)
 */
...
if (!checksum || !playerSteamId || !roundNumber || !demoPath) {
  console.error('用法: node cli-generate-frames-video.mjs --checksum <checksum> --player <steam_id> --round <number> --demo <path>');
  process.exit(1);
}

console.log('='.repeat(80));
console.log('CLI 视频版帧生成器（方案三：Video + Cache）');
console.log('='.repeat(80));
...
async function main() {
  ...
  try {
    ...
    const videoFileName = 'round_video';  // CS2 会自动添加 .avi
    ...
    const jsonContent = [
      {
        actions: [
          ...
          { tick: 1, cmd: 'volume 0' },  // 静音避免音频处理
          { tick: 1, cmd: 'cl_draw_only_deathnotices 0' },
          { tick: 1, cmd: 'spec_show_xray 0' },
          { tick: 1, cmd: `demo_gototick ${gotoTick} 1` },
          { tick: gotoTick + 5, cmd: 'demo_timescale 1' },  // 10倍速
          // 观察玩家
          { tick: gotoTick + 5, cmd: 'spec_mode 1' },
          { tick: gotoTick + 10, cmd: `spec_player x${task.player_steam_id}` },
          // 设置帧率和开始录制视频
          { tick: setupTick, cmd: 'host_framerate 25' },
          { tick: setupTick, cmd: `startmovie \"${videoFileName}\" avi` },  // 录制AVI视频
          // 结束录制
    ...
    // 查找视频文件（应该是 round_video.avi）
    const expectedVideoName = videoFileName + '.avi';
    const cs2VideoPath = path.join(cs2MoviePath, expectedVideoName);

    if (!await fs.pathExists(cs2VideoPath)) {
      console.error(`错误：未找到视频文件 ${cs2VideoPath}`);
      await client.end();
      process.exit(1);
    }

    console.log(`找到视频文件: ${cs2VideoPath}`);

    // 移动视频文件
    const finalVideoPath = path.join(outputPath, 'video.avi');
    await fs.move(cs2VideoPath, finalVideoPath, { overwrite: true });
    console.log(`✓ 视频已移动到: ${finalVideoPath}`);

    // 获取视频信息（使用 ffprobe）
    console.log('\n分析视频信息...');
    try {
      const { stdout: probeOutput } = await execAsync(
        `ffprobe -v error -select_streams v:0 -count_packets -show_entries stream=nb_read_packets,r_frame_rate -of csv=p=0 "${finalVideoPath}"`
      );
      const [frameRate, totalFrames] = probeOutput.trim().split(',');
      console.log(`视频信息: ${frameRate} fps, ${totalFrames} 帧`);
    } catch (error) {
      console.warn('⚠️ 无法获取视频信息（需要 ffprobe），继续...');
    }

    // 7. 生成帧索引（帧号 -> 时间戳映射）
    console.log('\n生成帧索引...');
    const totalExpectedFrames = windows.length * 10;
    const framesIndex = {};

    for (let i = 0; i < totalExpectedFrames; i++) {
      // 25fps，每帧 40ms
      framesIndex[i] = i * 0.04;  // 秒为单位
    }

    const framesIndexPath = path.join(outputPath, 'frames_index.json');
    await fs.writeJSON(framesIndexPath, framesIndex, { spaces: 2 });
    console.log(`✓ 帧索引已生成: ${framesIndexPath}`);
    ...
    const dataJson = {
      ...
      total_frames: totalExpectedFrames,

      // 视频存储模式
      storage_mode: 'video',
      video_path: `${outputFolder}/video.avi`,  // 相对路径
      frames_per_window: 10,
      fps: 25,

      windows: windows.map((w, idx) => {
        // 计算这个窗口对应的10帧（帧号）
        const startFrameIdx = idx * 10;
        ...
        return {
          ...
          // 存储帧号而不是路径
          start_frame_idx: frameIndices[0],
          middle_frame_indices: frameIndices.slice(1, 9),
          end_frame_idx: frameIndices[9],
    ...
    const estimatedJPGSizeMB = (totalExpectedFrames * 50 / 1024).toFixed(2);  // 假设每帧50KB
    const savingsPercent = (100 - (videoSizeMB / estimatedJPGSizeMB * 100)).toFixed(1);
    ...
    console.log('='.repeat(80));
    console.log(`输出目录: ${outputPath}`);
    console.log(`  - 视频文件: video.avi (${videoSizeMB} MB)`);
    console.log(`  - 窗口数量: ${windows.length}`);
    console.log(`  - 总帧数: ${totalExpectedFrames}`);
    console.log(`  - 存储节省: 约 ${savingsPercent}% (预估 ${estimatedJPGSizeMB} MB JPG -> ${videoSizeMB} MB 视频)`);
    console.log('');
