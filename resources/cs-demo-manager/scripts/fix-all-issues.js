...
// 修复 checkDataExists 函数
const oldCheckFunction = /\/\/ 检查数据是否已存在\nasync function checkDataExists\(checksum, playerSteamId, roundNumber\) \{[\s\S]*?^}/m;

const newCheckFunction = `// 检查数据是否已存在
...
content = content.replace(oldCheckFunction, newCheckFunction);

fs.writeFileSync(filePath, content, 'utf8');
console.log('✅ 修复完成！');
console.log('修复内容:');
console.log('1. 添加 let client = null 声明');
console.log('2. 添加 finally 块确保连接关闭');
console.log('3. SQL查询添加 GROUP BY player_name');
console.log('4. 使用可选链操作符 ?. 避免空值错误');
