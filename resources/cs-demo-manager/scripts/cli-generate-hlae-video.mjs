  ...
  try {
    ...
    if (await fs.pathExists(outputPath)) {
        try {
            ...
            // 判断是否完整
            if (videoExists && videoSize > 0 && framesDirExists && dataJson && frameCount > 0) {
                // 期望的帧数 = 训练窗口数 × 每窗口帧数
                const expectedFrameCount = windowsInDb.length * framesPerWindow;

                if (frameCount === expectedFrameCount && dataJson.windows && dataJson.windows.length === windowsInDb.length) {
                    isComplete = true;
                    console.log(`  ✓ 数据完整：视频 ${(videoSize / 1024 / 1024).toFixed(2)}MB，${frameCount} 帧，${dataJson.windows.length} 个窗口`);
                } else {
                    console.log(`  ⚠️ 数据不完整：帧数 ${frameCount}/${expectedFrameCount}，窗口 ${dataJson.windows?.length || 0}/${windowsInDb.length}`);
                }
            } else {
                console.log(`  ⚠️ 数据缺失：video=${videoExists}, frames=${framesDirExists}, data.json=${!!dataJson}, frameCount=${frameCount}`);
            }
        } catch (err) {
    ...
    console.log(`执行: ${hlaeCommand.substring(0, 100)}...`);
    console.log('');
    ...
    try {
      ...
      if (stderr) console.error('HLAE stderr:', stderr);

    } catch (error) {
      console.error('HLAE 录制失败:', error.message);
    ...
    for (const basePath of searchPaths) {
      if (await fs.pathExists(basePath)) {
        ...
        try {
            ...
            console.log(`  跳过目录 ${basePath}: ${e.message}`);
        }
      }
    }

    if (!sourceVideoPath) {
      console.error('❌ 未找到 HLAE 生成的视频文件');
      console.error('请手动检查 Demo 目录或 CSDM 输出目录');
      await client.end();
      process.exit(1);
    }
...
print(f"Total frames extracted: {frame_idx-1}")
    ...
    const dataJson = {
      ...
      windows: windows.map((w, index) => {
        // 计算该窗口对应的帧索引 (1-based)
        // 假设视频也是从 start_tick 开始录制的
        // start_tick 在视频中的时间 = (w.start_tick - startTick) / tickrate
        // 对应的帧号 = floor(time * fps) + 1

        const timeOffset = (w.start_tick - startTick) / tickrate;
        ...
        for (let i = 0; i < framesPerWindow; i++) {
            ...
            // 相对路径: player_steam_round/frames/frame_XXXX.jpg
            // 注意：这里需要根据用户的示例格式来调整
            // 用户示例: "-dyym_76561198883511494_round7/frames/frame_0001.jpg"
            // 也就是: outputFolder/frames/frame_XXXX.jpg
